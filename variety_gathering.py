...
variety_format = parse_formats.VarietyParseFormat()
print(f"\nScript for '{variety_format.host_name}' parsing.\n")
...
for min_page in range(variety_format.min_page_num, variety_format.max_page_num, delta_page):
  ...
  urls = parse_tools.gather_site_pages(min_page_num=min_page, max_page_num=min_page+1,
                                       categories=[cat_name], needle_URL_regexp=variety_format.needle_URL_regexp,
                                       hay_page_URL=variety_format.hay_page_URL)

  uid = parse_tools.dump_parsed_data_as_csv(output_filename=f"variety_{cat_name}_urls_records.csv",
